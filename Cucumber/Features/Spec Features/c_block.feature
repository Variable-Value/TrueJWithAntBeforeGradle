...
A block consists of variable declarations and any code that uses the variables or the value-names associated with them. We syntactically express a block by inclosing code between an opening curly brace { and a closing curly brace }. All executable code statements are contained in either a method's main block, a constructor's main block, or an intializer block. We use curly braces around other things, like the body of a class, but we only call it a "block" if the curly braces surround executable code. We start with an overview of blocks, and expand the explanations and give examples as part of the scenarios below.

(Some vocabulary: "Conjoin" is a handy verb for ANDing statements together. A "conjunction" is the result of conjoining statements.)
...
A block may be nested in another block's sequential code or used as part of a complex control statement, such as being one branch of a conditional statement. When a block is nested within another block, the inside block is treated as a single statement of the enclosing block. When a block is part of a complex control statement, then the flow of control does not always pass through the block, and this leads to a more complex meaning for the control statement, using the meaning of the block as a part of it.
...
To define or use a value of a variable we use a value name, constructing it from the variable's name using a decorator. A value name cannot be referenced unless all flows of execution define the value name and a value name cannot be referenced after a means-statement that does not reference the value. If a field is modified anywhere in a block, the code must define a final value for the field that is part of it the meaning of the block.
